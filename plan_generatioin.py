...
Format the response in clear sections with specific, actionable advice.
    ...
    def generate_fitness_plan(self, primary_user: UserProfile, matched_user: UserProfile) -> Dict[str, str]:
        ...
        """
        try:
            logger.info(f"Generating fitness plan for {primary_user.username} and {matched_user.username}")
    ...
    def save_fitness_plan(self, plan_data: Dict[str, str], filename: Optional[str] = None) -> str:
        ...
        """
        if filename is None:
            filename = f"fitness_plan_{plan_data['primary_username']}_{plan_data['matched_username']}.txt"
        ...
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                ...
                f.write("="*80 + "\n\n")
                f.write(f"Primary User: {plan_data['primary_username']}\n")
                f.write(f"Matched Partner: {plan_data['matched_username']}\n")
...
def main():
    ...
    try:
        ...
        print("="*80)
        print(f"Primary User: {plan_data['primary_username']}")
        ...
        print(f"Matched Partner: {plan_data['matched_username']}")
